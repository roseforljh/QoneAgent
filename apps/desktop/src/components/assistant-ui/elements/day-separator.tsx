...
export function DaySeparatorMarker({
  day,
  className,
  ...props
}: Omit<ComponentProps<"div">, "children"> & { day: string }) {
  return (
    <div
      data-slot="day-separator"
      className={cn("flex w-full max-w-sm items-center gap-2.5 py-1", className)}
      {...props}
    >
      ...
      {messages.map((message) => {
        ...
        return (
          <div key={message.id} className="flex flex-col gap-2">
            {newDay && <DaySeparatorMarker day={message.day} />}
            <div
              className={cn(
