...
export function MemoryChips({
  ...
  label?: string;
  forgetLabel?: (text: string) => string;
}) {
    ...
    >
      <div className="flex items-center gap-1.5">
        ...
        <span className={cn(mono, "text-foreground/35")}>
          {label ?? (fresh > 0 ? `remembered ${fresh}` : "memory")}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {chips.map((chip) => (
          ...
          >
            <span className="min-w-0 max-w-64 truncate" title={chip.text}>{chip.text}</span>
            {onForget && (
              <button
                type="button"
                aria-label={forgetLabel?.(chip.text) ?? `Forget "${chip.text}"`}
